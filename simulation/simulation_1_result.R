...
beta0 = matrix(NA,30,10000)
beta1 = matrix(NA,30,10000)
...
for (tt in 1:iter){
  print(tt)
  # load PD
  filename_control = list.files(path=paste0(getCurrentFileLocation(),"/simulation_1_pd")
                                ,pattern=paste0("control_iter_",tt-1,"_"),full.names = T)
  filename_test = list.files(path=paste0(getCurrentFileLocation(),"/simulation_1_pd")
