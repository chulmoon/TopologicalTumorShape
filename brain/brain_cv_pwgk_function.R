main=function(cc,clinical_data_brain,minmax0,minmax1,
  ...
  pfpwgk0 = as.data.frame(matrix(NA,nrow(patientid),pixnum0*(pixnum0+1)/2))
  pfpwgk1 =	as.data.frame(matrix(NA,nrow(patientid),pixnum1*(pixnum1+1)/2))
  ...
  for (ii in 1:nrow(patientid)){
    ...
    pfpwgk0[ii,] = dfmw0$newz/nslide # average density function	
    ...
    pfpwgk1[ii,] = dfpwgk1$newz/nslide # average density function	
  }
  ...
  rownames(pfpwgk0) = ids
  rownames(pfpwgk1) = ids
  X0=pfpwgk0
  X1=pfpwgk1
