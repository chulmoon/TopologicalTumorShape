main=function(cc,clinical_data_brain,minmax0,minmax1,
  ...
  pflinear0 = as.data.frame(matrix(NA,nrow(patientid),pixnum0*(pixnum0+1)/2))
  pflinear1 =	as.data.frame(matrix(NA,nrow(patientid),pixnum1*(pixnum1+1)/2))
  ...
  for (ii in 1:nrow(patientid)){
    ...
    # linear weight for dimension-zero
    linear0 = lin(pd0)
    ...
    pflinear0[ii,] = dfmw0$newz/nslide # average density function	
    ...
    pflinear1[ii,] = dflinear1$newz/nslide # average density function	
  }
  ...
  rownames(pflinear0) = ids
  rownames(pflinear1) = ids
  X0=pflinear0
  X1=pflinear1
