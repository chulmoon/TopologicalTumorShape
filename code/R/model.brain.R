...
# for here, set the directory pulled from https://github.com/lorinanthony/SECT
fdir="./SECT/Data/MITKSegmentations/"
ids=dir(fdir)
fdir2="/SECT/baseline/Segmentations/enh"
...
for (ii in 1:length(ids)){
	...
	for(jj in 1:length(fname)){
		# read images
		img=readPNG(paste0(fdir,"/",ids[ii],fdir2,"/",fname[jj]))
		# count tumor pixels
		ntumor = sum(sum(img))
	...
	idslice100mat=rbind(idslice100mat,ind)
}
...
# for here, set the directory pulled from https://github.com/chulmoon/TopologicalTumorShape
filename = list.files(path="./TopologicalTumorShape/data/brain/PersistenceDiagram/"
...
pd.total = pd.total %>% filter(slideid %in% idslice100mat$slideid, id1 %in% idslice100mat$id1, id2 %in% idslice100mat$id2)
...
##############################################################
# persistence diagram
#filename = list.files(path="C:/Users/Chul/Box Sync/r_Papers/ShapeAnalysis/Braindata/pd_rdata/"
...
hh=10 # up to 10 FPCs for each dimension
...
for (ss in 1:nrow(comb)) {
	...
	# FCoxPH model
	# overall survival
	inifor="Surv(os,event)~gender+age+ks"
...
#################################################################
# CoxPH model estimation
#################################################################
# model estimation
model.coxph = coxph(Surv(os,event)~age+ks+gender,data=comdf)
summary(model.coxph)
...
#################################################################
num.clinical = 3 # number of clinical variables
...
for (ii in 1:nrow(X0)){
	...
	for (ss in 1:nrow(comb)) {
		...
		# FCoxPH model
		## formula for FCoxPH model
		inifor="Surv(os,event)~age+ks+gender"
		for (s in 1:comb[ss,1]){
			inifor=paste0(inifor,paste0("+dim0.",s))
		}
		for (s in 1:comb[ss,2]){
	...
	inifor.coxph="Surv(os,event)~age+ks+gender"
...
survdiff(Surv(os, event) ~ group, data = risk.mean.fcoxph)

## KM plot
ggsurvplot(survfit(Surv(os, event) ~ group, data = risk.mean.fcoxph), 
...
risk.mean.coxph = comdf %>% 
	left_join(risk.df.coxph, by=c("id"="id"))
risk.mean.coxph$group=ifelse(risk.mean.coxph$risk<=median(risk.mean.coxph$risk),"low","high")

survdiff(Surv(os, event) ~ group, data = risk.mean.coxph)

## KM plot
ggsurvplot(survfit(Surv(os, event) ~ group, data = risk.mean.coxph), 
					 conf.int = TRUE, pval = T, pval.coord = c(1000, 0.9),
					 legend.labs = c("High", "Low"),legend.title="Predicted risk")
...
data.survdiff.fcoxph=survdiff(Surv(os, event) ~ group, data = risk.mean.fcoxph)
## hazard ratio
...
# CoxPH
## difference of curves in KM plot
data.survdiff.coxph=survdiff(Surv(os, event) ~ group, data = risk.mean.coxph)
## hazard ratio
