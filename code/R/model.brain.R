...
fdir="./SECT/Data/MITKSegmentations/" # data repository from https://github.com/lorinanthony/SECT
ids=dir(fdir)
...
for (ss in 1:nrow(comb)) {
  ...
  # designmatrix
  designmatrix=data.frame(eigenscore0,eigenscore1,ntumor=as.vector(ntumor),id=id_new)
  ...
  # FCoxPH model
  # overall survival
  inifor="Surv(os,event)~gender+age+ks+ntumor"
...
designmatrix=data.frame(eigenscore0,eigenscore1,as.vector(ntumor),id=id_new)
...
#################################################################
# model estimation
model.coxph = coxph(Surv(os,event)~age+ks+gender+ntumor,data=comdf)
summary(model.coxph)
...
## FCoxPH model fitting
model.fcoxph.var = coxph(as.formula(inifor),data=comdf_var, init = initial.value)
...
for (ii in 1:nrow(X0)){
  ...
  inifor.coxph="Surv(os,event)~age+ks+gender+ntumor"
...
## KM plot
KMplotCox = ggsurvplot(survfit(Surv(os, event) ~ group, data = risk.mean.coxph), 
                       conf.int = TRUE, pval = T, pval.coord = c(1000, 0.9),
                       legend.labs = c("High", "Low"),legend.title="Predicted risk")
