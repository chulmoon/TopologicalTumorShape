main=function(cc, clinical_info){
  ...
  pflinear0 = as.data.frame(matrix(NA,length(filename),pixnum0*(pixnum0+1)/2))
  pflinear1 = as.data.frame(matrix(NA,length(filename),pixnum1*(pixnum1+1)/2))
  ...
  for (ii in 1:length(filename)){
    ...
    pflinear0[ii,] = dfmw0$newz # persistence function	
    ...
    pflinear1[ii,] = dflinear1$newz # persistence function
  }
  
  
  # persistence function with the maximum distance weight
  rownames(pflinear0) = slideid
  rownames(pflinear1) = slideid
  X0=pflinear0
  X1=pflinear1
